...
class RTCPeerConnection extends SimpleEventTarget {
  ...
  _queuePendingRemoteCandidatesForNative(candidates) {
    for (const candidate of candidates) {
      ...
      if (this._remoteIceCandidates.some((entry) => sameCandidate(entry, candidate))) continue;
      if (this._nativeRemoteCandidates.some((entry) => sameCandidate(entry, candidate))) continue;
      if (
  ...
  _flushPendingRemoteCandidatesForNative() {
    ...
    for (const candidate of candidates) {
      ...
      if (!this._nativeRemoteCandidates.some((entry) => sameCandidate(entry, candidate))) {
        this._nativeRemoteCandidates.push(candidate);
      }
    }
  }
